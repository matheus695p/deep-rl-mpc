...
df = pd.read_csv("data/data_energy_train.csv").reset_index(drop=True)
test_df = pd.read_csv("data/data_energy_test.csv").reset_index(drop=True)

# dataframe atributes
target = "pe"
control_variables = ["at", "v"]
context_variables = ["ap", "rh"]
...
# actor critic agents
agent = ACKTR(MlpPolicy, env, verbose=1, tensorboard_log="./tensorboard/")
agent.learn(total_timesteps=max_episodes, callback=callback)
agent.save(f"agents/{model_name}")
# reload agent
agent = ACKTR.load(f"agents/{model_name}")
